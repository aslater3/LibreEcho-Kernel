...
class V157DiagnosticShapeTests(unittest.TestCase):
    ...
    def test_clearing_assert_state_resets_capture_budget(self):
        ...
        self.assertIn("atomic_read(&echo_fw_assert_latched)", self.stp[get_start:get_end])
    ...
    def test_capture_runs_only_at_complete_frame_boundaries(self):
        ...
        crc_success = self.stp[crc_start:crc_end]
        self.assertIn("echo_stp_capture_frame", crc_success)
        ...
        fw_complete = self.stp[complete_start:complete_end]
        self.assertIn("echo_stp_capture_frame", fw_complete)
    ...
    def test_start_command_bytes_are_logged_once(self):
        ...
        submit = self.nic_tx[start:end]
        self.assertIn("ECHO_FW_START_CMD", submit)

        start = self.wlan.index("WLAN_STATUS wlanConfigWifiFunc")
        end = self.wlan.index("UINT_32 wlanCRC32", start)
        function = self.wlan[start:end]
        self.assertIn("no start-command ACK is waited", function)
    ...
    def test_ready_loop_aborts_on_stp_firmware_assert(self):
        ...
        self.assertIn("mtk_wcn_stp_coredump_start_get()", ready_loop)
